...
int main(int argc, char **argv) {
    ...
    if (argc != 3) {
        fprintf(stderr, "Usage: %s [filename.mp3] [device name]", argv[0]);
        return 255;
    }
...
//    mad_header_decode(&mad_header,&mad_stream);
//    printf("Bitrate %u\n",mad_header.samplerate);
    if(!alsaInit(argv[2], 44100)) exit(1);
