...
static TimerHandle_t _powerTimer = NULL;
...
static void _selectOutput(uint8_t num) {
    ...
    cmdUartSendByte(num+CMD_PREFIX_CH_SEL);
}
...
// подаем питание на  прогрев ламп и запускаем таймер прогрева
void _startWarming(void){
    int id =1;
    cmdUartSendByte(CMD_PREFIX_PWR+PWR_BIT_LAMP);// Включаем питание ламп
    _powerTimer =  xTimerCreate("PowerTimer",10000 / portTICK_RATE_MS,pdFALSE,&id,vPowerTimerCallback);
    xTimerStart(_powerTimer, 0);
    ST7735_FillRectangle(0,0,100,ST7735_HEIGHT,ST7735_BLACK);
    ST7735_WriteString(2,2,"Warming...",Font_11x18,ST7735_RED,ST7735_BLACK);
}
...
void _powerOff(void) {
    ...
    if(!_powerState) return;
     ...
     displayOff();
    btOff();
     cmdUartSendByte(CMD_PREFIX_PWR + 0);// Выключаем питание усилителя
     _powerState = false;
     _ready = false;
}
